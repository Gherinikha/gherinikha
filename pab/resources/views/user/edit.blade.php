...
<ul>
 ...
 @endif
 <form class="w-full" action="{{ url('/user/update/'.$item->id) }}"
method="post">
 @csrf
...
<input value="{{ old('name') ?? $item->name }}"
name="name" class="appearance-none block w-full bg-gray-200 text-gray-700 border 
